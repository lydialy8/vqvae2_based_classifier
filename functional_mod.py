...
def to_tensor(img):
    ...
    Arguments:
        img {numpy.ndarray} -- image to be converted to tensor. tf.float32
    """
...
def normalize(tensor, mean, std, inplace=False):
    ...
    mean = tf.Tensor(mean, dtype=tensor.dtype, device=tensor.device)
    std = tf.Tensor(std, dtype=tensor.dtype, device=tensor.device)
...
def noise(img, mode="gaussain", percent=0.02):
    ...
    if mode == "gaussian":
        ...
        if img.ndim == 2:
            ...
            b, h, w, c = img.shape
            gauss = np.random.normal(mean, sigma, (b, h, w, c))
        else:
            h, w, c = img.shape
